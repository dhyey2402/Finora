...
import { Loader2Icon, BanknoteIcon, PlusIcon, Trash2Icon } from "lucide-react";
...
const saleSchema = z.object({
  ...
  sale_number: z.string().min(1, "Voucher No is required"),
...
export function SaleFormDialog({
  ...
  useEffect(() => {
    if (open) {
      reset({
        customer_id: "",
        sale_number: `SAL-${Date.now().toString().slice(-6)}`,
        sale_date: new Date().toISOString().split("T")[0],
        notes: "",
  ...
  const createMutation = useMutation({
    mutationFn: (data: SaleCreate) => saleService.create(companyId, data),
    onSuccess: () => {
      toast.success("Sale recorded successfully");
      queryClient.invalidateQueries({ queryKey: ["sales", companyId] });
      // Invalidate stock items since inventory decreased
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Record a new sale voucher with line items.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 py-4">
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="space-y-2 col-span-1">
              ...
              <Input disabled={isLoading} {...register("sale_number")} />
              {errors.sale_number && (
                <p className="text-xs text-destructive">{errors.sale_number.message}</p>
              )}
            </div>
            ...
            <div className="space-y-2 col-span-1">
              ...
              <Input type="date" disabled={isLoading} {...register("sale_date")} />
              {errors.sale_date && (
